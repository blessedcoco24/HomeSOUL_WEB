...
export class ContentComponent {
  ...
  cart: CartItem[] = [];
  private totalPricepay: number = 0;
  ...
  goToOrder() {
    ...
    const totalPrice = selectedItems.reduce((sum, item) => sum + (item.totalPrice || 0), 0);
    this.cartService.setSelectedItems(selectedItems);
  
    this.router.navigate(['/order'], { queryParams: { total: totalPrice } });
  }
  ...
  setTotalPrice(price: number) {
    this.totalPricepay = price;
  }

  getTotalPricepay(): number {
